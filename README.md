...
| `02_Auth.js` | 固定IDセッションの受付、移行用メール認証、管理者判定、本人／管理者の更新権限 |
...
| `GOOGLE_IDENTITY_CLIENT_ID` | Googleログイン用OAuth 2.0 WebクライアントID |
| `GOOGLE_IDENTITY_CLIENT_SECRET` | Googleログイン用OAuth 2.0 Webクライアントシークレット |
| `APP_SESSION_SECRET` | OAuth state署名鍵。初回ログイン時に自動生成 |
...
- 実行ユーザー: Webアプリをデプロイした運用アカウント（`USER_DEPLOYING`）
- Google側のアクセス範囲: ログイン済みGoogleアカウント（`ANYONE`）
- アプリ側のアクセス範囲: 初回に設定メールと照合し、固定Google IDを連携した有効メンバーと設定済み管理者

データの読み書きはデプロイ用アカウントの権限で行うため、一般利用者にはスプレッドシート等のOAuth権限承認画面を表示しません。GitHub Actionsでは`CLASPRC_JSON`に保存したGoogleアカウントがデプロイ用アカウントになるため、このアカウントへ参照先ブックの権限を付与してください。

初回ログインではGoogleのIDトークンから固定IDである`sub`を取得し、設定タブのメールと一度だけ照合します。対応関係は`IdentityBindings`へ保存します。同時にApps Scriptの一時ユーザーキーをハッシュ化して`IdentitySessions`へ固定IDとの対応を保存し、2回目以降はメールアドレスではなくこの対応で判定します。ドメイン名による判定は行いません。

GoogleログインのクライアントIDとシークレットが未設定の間だけ、管理者が設定を完了できるよう従来の`Session.getActiveUser()`判定を移行用に残します。両方を保存した時点で旧判定は停止し、全利用者が固定Google ID認証へ切り替わります。

Googleログインの認可コードとクライアントシークレットはサーバー側だけで処理します。ブラウザへ認証トークンは保存しません。一時ユーザーキーはGoogleの仕様により30日周期で変わるため、最大30日以内に再度Googleログインを要求します。シートへ保存するのは一時キーそのものではなくSHA-256ハッシュです。

Apps Scriptの画面はiframe内で動くため、最上位画面のURLをJavaScriptから直接変更しません。ログイン開始は事前生成した`target="_top"`リンクで行い、OAuth完了時はコールバック応答からアプリ本体を直接描画します。

メンバー設定を保存すると、有効メンバーのメール一覧が`AUTHORIZED_EMAILS`へ同期されます。一般利用者へ参照先スプレッドシートの共有権限は自動付与しません。設定から削除・無効化されたユーザーはアプリへ入れなくなります。

以前の方式で利用者へ付与済みのスプレッドシート共有権限は、自動では削除しません。不要な場合は各スプレッドシートの共有設定から手動で削除してください。

移行後に`AUTHORIZED_EMAILS`が存在しない場合は、設定済み管理者が最初にアクセスした時点で既存の有効メンバーから自動生成されます。

### Google固定IDログインの初期設定

1. Google Cloudの「Google Auth Platform」でOAuth同意画面を作成します。外部Gmailも利用するため、対象は「外部」を選びます。
2. 公開前のテスト状態では、利用するGmailアカウントをテストユーザーへ追加します。
3. OAuthクライアントを「ウェブ アプリケーション」で作成します。
4. 設定画面に表示される「承認済みリダイレクトURI」を、Cloud側の承認済みリダイレクトURIへ完全一致で登録します。
5. 発行されたクライアントIDとクライアントシークレットを、管理画面の設定タブへ保存します。
6. 対象ユーザーが初回ログインすると、`IdentityBindings`シートへGoogle固定IDとメンバーIDの対応が保存されます。

設定画面へ入れない状態で初期設定する場合は、Apps Scriptの「プロジェクトの設定 → スクリプト プロパティ」へ`GOOGLE_IDENTITY_CLIENT_ID`と`GOOGLE_IDENTITY_CLIENT_SECRET`を直接登録してください。リダイレクトURIは現在の本番WebアプリURL（`.../exec`）です。GitHub Actionsで既存のデプロイIDを更新する限りURLは変わりません。

メンバーのGoogleアカウントを変更する場合は、設定タブの「Google認証 → 再設定」を押してから、新しいGoogleアカウントでログインします。
