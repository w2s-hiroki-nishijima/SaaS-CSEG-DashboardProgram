...
| `02_Auth.js` | ドメイン制限、管理者判定、本人／管理者の更新権限 |
...
| `ALLOWED_DOMAIN` | 利用を許可するGoogle Workspaceドメイン |
...
- 実行ユーザー: Webアプリをデプロイしたユーザー（`USER_DEPLOYING`）
- アクセス範囲: デプロイユーザーと同じGoogle Workspaceドメイン（`DOMAIN`）

利用者本人のメールアドレスは`Session.getActiveUser()`で判定し、スプレッドシート操作だけをデプロイユーザーの権限で実行します。これにより、各利用者へ内部データブックを直接共有せずに画面を利用できます。

デプロイを担当するユーザーには、次の3ブックの閲覧・編集権限が必要です。
